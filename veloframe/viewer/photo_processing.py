...
def load_photo(photo_path: str, screen_size: Optional[Tuple[int, int]] = None, apply_blur_background: bool = False) -> Tuple[Image.Image, Dict]:
    ...
    try:
        ...
        # Create a small blank image as a fallback
        if screen_size:
            width, height = screen_size
        else:
            width, height = 800, 600
        blank_img = Image.new('RGBA', (width, height), (0, 0, 0, 255))
        return blank_img, {}
...
def create_blurred_background(image: Image.Image, screen_size: Tuple[int, int]) -> Image.Image:
    ...
    try:
        ...
        if scaled_size[0] < screen_width or scaled_size[1] < screen_height:
            ...
            # Clean up intermediate images to prevent memory leaks
            foreground.close()
            del foreground
...
def cleanup_memory():
    """Force cleanup of PIL image memory and suggest garbage collection."""
    # Close any unclosed PIL images (though this shouldn't happen with proper context management)
